    ...
    on<GetBrandTermsEvent>((event, emit) async {
      ...
      result.fold(
        ...
        emit(state.copyWith(
            //state
            brandsTerms: r,
            searchedBrandsTerms: r,
            brandsTermsState: RequestState.loaded));
      });
    });
    //
    on<SearchBrands>((event, emit) async {
      print('SearchBrands');
      emit(state.copyWith(brandsTermsState: RequestState.loading));
      List<Term> searchedbrands = state.brandsTerms
          .where(
      ...
      emit(state.copyWith(
          //state
          searchedBrandsTerms: searchedbrands,
          brandsTermsState: RequestState.loaded));
      print(state.searchedBrandsTerms);
    });

    on<SearchStores>((event, emit) async {
      print('SearchStores');
      List<Term> searchedstores = state.allStores
          .where(
            (element) => element.name.contains(event.text),
          )
          .toList();
      ...
      print(state.searchedStores);
    });

//
    on<SearchRestaurants>((event, emit) async {
      print('SearchRestaurants');
      List<Term> allRestaurants = state.allRestaurants
          .where(
            (element) => element.name.contains(event.text),
          )
          .toList();
      ...
      emit(state.copyWith(
        //state
        searchedRestaurants: allRestaurants,
      ));
      print(state.searchedRestaurants);
    });
    //
    on<GetClothingTermsEvent>((event, emit) async {
      ...
      result.fold(
        ...
        emit(
          state.copyWith(
            allStores: List.from(state.allStores)..addAll(r),
          ),
        );
        emit(state.copyWith(
            //state
    ...
    on<GetlibraryTermsEvent>((event, emit) async {
      String? terms = Preferences.getLibrary();
      if (terms != '' && terms != null) {
        var myTerms = await jsonDecode(terms);
        myTerms = List<TermModel>.from((myTerms).map(
          (e) => TermModel.fromJson(e),
    ...
    on<GetmassTermsEvent>((event, emit) async {
      String? terms = Preferences.getMass();
      if (terms != '' && terms != null) {
        var myTerms = await jsonDecode(terms);
        myTerms = List<TermModel>.from((myTerms).map(
          (e) => TermModel.fromJson(e),
    ...
    on<GetFastFoodTermsEvent>((event, emit) async {
      emit(state.copyWith(fastfoodTermsState: RequestState.loading));
      ...
      result.fold(
        ...
        emit(
          state.copyWith(
            allRestaurants: List.from(state.allRestaurants)..addAll(r),
          ),
        );
        emit(state.copyWith(
            //state
  ...
  void addAllStores({required bool isRefresh}) {
    ...
    add(GetlibraryTermsEvent(
        pageNum: 1, attributeId: 37, perPage: 100, isRefresh: isRefresh));
    add(GetmassTermsEvent(
        pageNum: 1, attributeId: 38, perPage: 100, isRefresh: isRefresh));
  }
